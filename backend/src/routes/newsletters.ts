...
router.post('/generate', async (req: AuthRequest, res) => {
  try {
    ...
    const newsletter = await generateNewsletter(req.userId!);
    console.log('Newsletter generated successfully with', newsletter.events.length, 'events');
    res.json(newsletter);
  } catch (error) {
    console.error('Newsletter generation error:', error);
    console.error('Error stack:', error instanceof Error ? error.stack : 'No stack trace');
    if (error instanceof Error) {
      return res.status(400).json({ error: error.message, details: process.env.NODE_ENV === 'development' ? error.stack : undefined });
    }
    res.status(500).json({ error: 'Internal server error' });
  }
});
...
router.post('/:id/send', async (req: AuthRequest, res) => {
  try {
    ...
    console.log('Send newsletter request:', { newsletterId, userId: req.userId });
